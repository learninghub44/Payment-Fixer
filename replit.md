...
Admin login is seeded from environment variables `SEED_ADMIN_EMAIL` and `SEED_ADMIN_PASSWORD`.
...
The seed creates an admin user (from `SEED_ADMIN_EMAIL` / `SEED_ADMIN_PASSWORD` env vars) and default leaders.
